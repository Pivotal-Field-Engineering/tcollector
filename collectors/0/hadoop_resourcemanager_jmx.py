...
JMX_SERVICE_RENAMING = {
  "GarbageCollector": "resourcemanager.gc",
  "OperatingSystem": "resourcemanager.os",
  "Threading": "resourcemanager.threads",
}

IGNORED_METRICS = set(["revision", "date", "user", "url", "version",
                       "DamenodeAddress", "Version", "RpcPort", "HttpPort",
...
def main(argv):
    ...
    jmx = subprocess.Popen(
        ["java", "-enableassertions", "-enablesystemassertions",  # safe++
         ...
         "ResourceManager",  # Name of the process.
    ...
    try:
        ...
        while True:
            ...
            # Only pass numeric values
            try:
                float(value)
            except ValueError, e:
                print >>sys.stderr, ("Invalid value on line: %r -- %s"
                                     % (line, e))
                continue
